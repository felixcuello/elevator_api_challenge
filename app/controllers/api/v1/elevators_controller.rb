...
module Api
  module V1
    class ElevatorsController < ApiController
      ...
      def move_down
        if @elevator.top_floor?
          MoveDownWorker.perform_async({ elevator_id: @elevator.id, floors: @building.floors }.stringify_keys)
      ...
      def move_up
        if @elevator.ground_floor?
          MoveUpWorker.perform_async({ elevator_id: @elevator.id, floors: @building.floors }.stringify_keys)
